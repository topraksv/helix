...
description: Provides guidance for property-based testing of serialization, validation, parsing, normalization, ordering, calculations, and state invariants. Use when designing or testing Helix transformations whose input space is wider than a small set of examples.
---
...
TASK: Writing new tests
  → Read [references/generating.md](references/generating.md)
  → Then [references/strategies.md](references/strategies.md) if input generation is complex

TASK: Designing a new feature
  → Read [references/design.md](references/design.md)

TASK: Code is difficult to test (mixed I/O, missing inverses)
  → Read [references/refactoring.md](references/refactoring.md)

TASK: Reviewing existing PBT tests
  → Read [references/reviewing.md](references/reviewing.md)

TASK: Test failed, need to interpret
  → Read [references/interpreting-failures.md](references/interpreting-failures.md)
...
> "`encode_message`/`decode_message` is a serialization pair. A roundtrip
> property would cover more inputs than a few examples. I will use that
> approach if it fits the existing test stack."
...
- **"The test failed, so it's a bug"** - Failures require validation; see
  [references/interpreting-failures.md](references/interpreting-failures.md)
