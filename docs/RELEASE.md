...
Linked şema `…01`–`…09` için senkrondur; paket branch’indeki
`…10_tombstone_only_client_deletes` ve `…11_category_transfer_semantics`
protected merge sonrasını bekler. Dry-run yalnız bu iki migration’ı listeler;
erken uygulanmaz. Merge sonrası liste/lint/pgTAP/type-generation tamamlanınca bu
paragraf gerçek remote durumuyla güncellenir.
