...
The 2026-07-29 import-graph audit found no static module cycle. The one
deliberate runtime back-edge is `components → calculator → components`:
`AmountField` resolves the calculator with a deferred `require`, after
`components.tsx` has initialized. It does not split the Metro bundle; moving
`Button` and `FadeIn` solely to make a cycle tool quiet would widen the shared
primitive interface without runtime or bundle benefit, so the evaluated-safe
seam stays documented beside `LazyCalculatorModal`.
