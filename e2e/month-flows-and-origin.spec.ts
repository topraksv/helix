...
test("a future month focus states the planned flows behind its own total", async ({ page }) => {
  ...
  // card states it, and the ledger's own "Güncel Bakiye" column states it too. The
...
test("Analysis returns to Summary even after a detour through Budgets @smoke @cross-browser", async ({ page }) => {
  await onboard(page);
