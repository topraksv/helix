...
export const tr = {
  ...
  subs: {
    ...
    scheduleOverviewHint: "Önündeki 31 günün ödeme yoğunluğu.",
    tryMonthlyLoad: "TRY aylık karşılığı",
    dayWindow: (days: number) => `${days}. gün`,
    dueSummary: (count: number) => `${count} yaklaşan ödeme`,
    automaticCoverage: (automatic: number, manual: number) => `${automatic} otomatik · ${manual} elle takip`,
    scheduleOverviewA11y: (active: number, upcoming: number, automatic: number, density: number[]) =>
      `${active} abonelik; 31 günde ${upcoming} ödeme; ${automatic} otomatik. 7 günlük aralıklar: ${density.join(", ")}.`,
