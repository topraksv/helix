...
export const tr = {
  ...
  legal: {
    ...
    transferNote: "Türkiye dışına yapılan bu aktarımlar KVKK m. 9 hükümlerine tabidir. Helix hesap açmadan kullanılamadığı için bu aktarımların dışında kalmanın bir yolu yoktur; kabul etmiyorsanız hesap oluşturmamanız gerekir. Aktarımın kapsamı ise sınırlıdır: kayıtlarınız ve belgeleriniz yalnızca Supabase'e; Helix'in gönderdiği e-postalar ve geri bildiriminiz yalnızca Google'a (Gmail) gider. Kur, piyasa ve logo servislerine giden isteklerde kayıtlarınız yoktur — yalnızca bağlantı bilginiz ve, logo isteğinde, yazdığınız kurumun alan adı.",
  ...
  errors: {
    ...
    workbookTooLarge: "Kayıtların tek bir Excel dosyasına sığmayacak kadar çok. Tam yedek al: JSON yedeği bu sınıra takılmaz.",
  ...
  auth: {
    ...
    resetExpiredBody: "Bu bağlantının süresi dolmuş ya da daha önce kullanılmış. Yenileme bağlantıları kısa süre geçerli ve tek kullanımlık; giriş ekranından yeni bir tane iste.",
    ...
    resetOtherTabNote: "Helix başka bir sekmede açık. Devam etmek için o sekmeye dön.",
  ...
  dashboard: {
    ...
    cardStatementPaid: (paid: string) => `${paid} ödendi`,
    late: "Geciken",
  ...
  cardStatement: {
    title: "Kart Ekstresi",
    card: "Kart",
    noCards: "Kredi kartın yok",
    noCardsHint: "Ekstre ödemesi kaydetmek için önce kendi kredi kartını ödeme yöntemi olarak ekle.",
    noStatements: "Bu ay için ekstre yok",
    noStatementsHint: "Karta harcama girdiğinde o ayın ekstresi kendiliğinden oluşur.",
    periodTitle: (card: string, month: string) => `${card} · ${month} ekstresi`,
    dates: (closing: string, due: string) => `Kesim ${closing} · Son ödeme ${due}`,
    charges: "Ekstre",
    paid: "Ödenen",
    remaining: "Kalan",
    state: { full: "Tam ödendi", partial: "Kısmen ödendi", minimum: "Asgari ödendi" },
    stateAssumed: "Ödendi sayıldı",
    stateOpen: "Ödeme bekliyor",
    assumptionHint: "Ödeme kaydı yoksa ekstre son ödeme gününde tamamen ödenmiş sayılır; harcamalar o gün bakiyenden düşer.",
    fullHint: (day: string) => `Ekstre ${day} tarihinde tamamen ödendi; harcamalar o gün bakiyenden düştü.`,
    owedHint: (left: string) => `Bakiyenden yalnız ödediğin tutar düştü. Kalan ${left} karta borç olarak görünür; faiz ve sonraki ekstreye aktarım bankanın hesabıdır.`,
    chargesTitle: "Ekstredeki Harcamalar",
    chargesEmpty: "Bu ekstrede harcama yok.",
    paymentsTitle: "Ödemeler",
    paymentsEmpty: "Bu ekstre için ödeme kaydı yok.",
    addTitle: "Ödeme Kaydet",
    addHint: "Ekstreyi kendin ödediysen kaydet. İlk kayıttan sonra bu ekstre için otomatik ödeme yapılmaz: tam ödemede harcamalar ödediğin güne taşınır, kısmi ödemede bakiyenden yalnız ödediğin tutar düşer.",
    kindFull: "Tam Ödeme",
    kindFullHint: (left: string) => `Kalan ${left} ödenir.`,
    kindMinimum: "Asgari Ödeme",
    kindMinimumHint: "Bankanın istediği en az tutar.",
    kindPartial: "Kısmi Ödeme",
    kindPartialHint: "Kalanın bir kısmı.",
    amount: "Ödenen tutar",
    paidOn: "Ödeme tarihi",
    note: "Not (isteğe bağlı)",
    save: "Ödemeyi Kaydet",
    nothingLeft: "Bu ekstrede ödenecek tutar kalmadı.",
    tooLarge: (left: string) => `Bu ekstrede ödenecek ${left} kaldı; daha fazlası kaydedilemez.`,
    saved: "Ekstre ödemesi kaydedildi",
    deleted: "Ekstre ödemesi silindi",
    showInstallments: "Karttaki Taksitleri Gör",
  },
  cell: {
    title: "Hücre Detayı",
    total: "Hücre toplamı",
  ...
  cashflow: {
    ...
    cardPayments: "Ekstre ödemesi", cardOwed: "Karta kalan borç",
    statementPaidOn: (day: string) => `Ekstresi ${day} ödendi`,
  ...
  tx: {
    ...
    cardMonthAndDue: (month: string, due: string) => `${month} ekstresi · Son ödeme ${due}`,
    cardMonthOnlyHint: (month: string, statement: string, due: string) =>
      `Tarihsiz — ${month} ekstresine (kesim ${statement}) eklenir; bakiyene ${due} son ödeme tarihinde yansır.`,
    // Shown when an expense paid from an account looks like a card bill while
    // the owner has a card of their own: the card's charges already leave the
    // balance on its due date, so this row would take the same money twice.
    cardBillTitle: "Kart ekstresini mi ödüyorsun?",
    cardBillWarning:
    ...
    refundTooLarge: (left: string) => `İade, harcamanın kalan tutarını (${left}) aşamaz.`,
    refundsTitle: "İadeler",
    refundsHint: "Bu harcamaya bağlı iadeler. Her iade kendi ayında ayrı bir satır olarak kalır.",
    refundsEmpty: "Bu harcamaya bağlı iade yok.",
    refundAdd: "İade Ekle",
    refundsPurchase: "Harcama",
    refundsTotal: "İade",
    refundsNet: "Kalan",
    ...
    /** When the total does not divide evenly the FIRST instalment carries the
     *  kuruş difference, which is how most statements bill it. Saying only the
     *  uniform figure made the preview add up to less than the purchase. */
    installmentInfoUneven: (n: number, first: string, rest: string) =>
      `İlk taksit ${first}, sonraki ${n - 1} taksit ${rest}`,
  ...
  installments: {
    ...
    monthlyTotalInfo: (n: number, monthly: string, total: string) => `${n} taksit × ${monthly} · toplam ${total}`,
    byCategoryTitle: (month: string) => `${month} · Kategorilere Göre`,
    refundAdd: "İade Ekle",
    refundTitle: "Taksit İadesi",
    refundPickTitle: "Hangi alışverişi iade ettin?",
    refundPickHint: "İade ettiğin ürünün bulunduğu taksitli alışverişi seç. İade o alışverişe bağlanır, kalan tutarı kendiliğinden güncellenir.",
    refundSearch: "Alışveriş ya da kart ara",
    refundPickEmpty: "Aramana uyan taksitli alışveriş yok",
    refundPickEmptyHint: "Başka bir kelimeyle ara; iadesi tamamlanmış alışverişler bu listede görünmez.",
    refundPlanLine: (card: string, total: string, paid: number, count: number) => `${card} · ${total} · ${paid}/${count} ödendi`,
    refundedSoFar: (amount: string) => `${amount} iade edildi`,
    refundAmount: "İade tutarı",
    refundAmountNote: "Aynı alışveriş için birden fazla iade geldiyse toplamını yaz.",
    refundSpreadLabel: "Bankan iadeyi nasıl yansıtıyor?",
    refundSpreadRemaining: "Kalan Taksitlere Bölünüyor",
    refundSpreadRemainingHint: (n: number, share: string | null) =>
      share ? `Kalan ${n} taksitin her birinden ${share} düşer.` : `Kalan ${n} taksite eşit bölünür.`,
    refundSpreadRemainingNone: "Ödenmemiş taksit kalmadı.",
    refundSpreadOnce: "Tek Seferde",
    refundSpreadOnceHint: "Seçtiğin ayın ekstresine tek satır olarak düşer.",
    refundMonth: "Hangi ayın ekstresinde?",
    refundNote: "Not (isteğe bağlı)",
    refundPreview: (before: string, after: string) => `Alışveriş tutarı ${before} → iade sonrası ${after}`,
    refundSave: "İadeyi Kaydet",
    refundChangePlan: "Başka Alışveriş Seç",
    refundTooLarge: (left: string) => `İade, alışverişin kalan tutarını (${left}) aşamaz.`,
    refundNothingLeft: "Bu alışverişin ödenmemiş taksiti kalmadı; iadeyi tek seferde gir.",
    refundBadge: (amount: string) => `İade ${amount}`,
    statementState: { full: "Ekstre ödendi", partial: "Ekstre kısmen ödendi", minimum: "Asgari ödendi" },
    dueDayField: "Ödeme günü",
    dueDayHint: "Taksitin ayın hangi günü ödendiği. Boş bırakırsan ödeme yönteminin günü, o da yoksa ayın 1'i kullanılır.",
    closedBadge: "Kapatıldı",
    closedTitle: "Kredi kapatıldı",
    closedLine: (day: string, payoff: string | null) =>
      payoff ? `${day} tarihinde ${payoff} ödenerek kapatıldı.` : `${day} tarihinde kapatıldı.`,
    closedEditLocked: "Kapatılmış kredi düzenlenemez. Bir şey yanlışsa önce kapatmayı geri al.",
    closeTitle: "Krediyi Kapat",
    closeHint: "Krediyi erken kapattıysan kapatma gününü ve bankaya ödediğin kapama tutarını gir. O günden sonraki taksitler kaldırılır, kapama tutarı o gün bakiyenden düşer. Yanlışlıkla kapatırsan geri alabilirsin.",
    closedOnField: "Kapatma tarihi",
    payoffAmount: "Kapama tutarı",
    closeNote: "Not (isteğe bağlı)",
    closePreview: (removed: number) => (removed === 0 ? "Kaldırılacak taksit yok." : `Bu tarihten sonraki ${removed} taksit kaldırılacak.`),
    closeBeforeFirst: "Kapatma tarihi ilk taksitten önce olamaz; hiç ödenmemiş bir krediyi silebilirsin.",
    closeAction: "Krediyi Kapat",
    closeConfirmTitle: "Kredi kapatılsın mı?",
    closeConfirmBody: (day: string, payoff: string, removed: number) =>
      `${day} tarihinde ${payoff} ödenerek kapatılacak${removed > 0 ? `, sonraki ${removed} taksit kaldırılacak` : ""}. Daha sonra geri alabilirsin.`,
    closedNotice: "Kredi kapatıldı",
    ...
    restructureMonthly: "Yeni aylık taksit",
    restructureRemaining: "Kalan taksit sayısı",
    restructurePreview: (n: number, monthly: string, end: string) => `Kalan ${n} taksit × ${monthly}; son taksit ${end}.`,
    restructureAction: "Yapılandırmayı Kaydet",
    restructuredNotice: "Kredi yeniden yapılandırıldı",
    refundsTitle: "İadeler",
    refundsHint: "Bu alışverişe bağlı iadeler. Birini silersen güncel tutar yeniden hesaplanır.",
    refundsEmpty: "Bu alışverişe bağlı iade yok.",
    refundDeleteTitle: "İade silinsin mi?",
    refundDeleteBody: (amount: string) => `${amount} tutarındaki iade silinecek; güncel tutar yeniden hesaplanır.`,
    purchaseTotal: "Alışveriş",
    refundsTotal: "İade",
    netTotal: "Güncel Tutar",
    rescheduleNote: "Kaydettiğinde bütün taksitler bu başlangıca göre yeniden dizilir; ödendi görünen aylar da buna göre değişir.",
    count: "Taksit sayısı",
  ...
  analysis: {
    ...
    remainderTitle: "Excel kolon farkları",
    remainderHint: "Excel'den gelen bir kolonun, taksit listesi dışında kalan kısmı. Kolon toplamını dosyayla aynı tutar ve bakiyede yer alır; harcama olmadığı için grafiklere, kategori tablosuna ve limitlere katılmaz.",
    remainderBadge: "Excel kolon farkı",
  },
  settings: {
    title: "Ayarlar",
    ...
    declarationTitle: "Ay Başı Bakiyesi",
    declarationHint: "Bir ayın hangi bakiyeyle başladığını biliyorsan yaz. O ay bu bakiyeyle açılır; öncesine sonradan eklediğin kayıtlar o ayı ve sonrasını değiştirmez.",
    declarationAmount: (month: string) => `${month} başındaki bakiye`,
    declarationSave: "Ay Başı Bakiyesini Kaydet",
    declarationSaved: "Ay başı bakiyesi kaydedildi",
    declarationUnavailable: "Başlangıç ayından sonraki aylar için yazılabilir; başlangıç ayının bakiyesini aşağıdaki geçmiş bölümünden değiştir.",
    declarationRow: (month: string) => `${month} başı bakiyesi`,
    declarationRowHint: (declared: string) => `${declared} olarak belirtildi; tutar bugünkü farkı gösterir`,
    ...
    exportWorkbook: "Excel'e Aktar",
    exportWorkbookDesc: "Mali tablon her yıl kendi sayfasında, aboneliklerin ve yatırımların ayrı sayfalarda. Excel'de düzenleyip Mali Tablo'yu buradan geri yükleyebilirsin; abonelik ve yatırım sayfaları okumak içindir.",
  ...
  importer: {
    ...
    templateTitle: "Elinde uygun bir tablo yok mu?",
    templateLead: "Hazır şablonu indir, kendi rakamlarınla doldur, buradan geri yükle. Bir yıllık boş bir bütçe tablosu — aylar alt alta, kalemler yan yana.",
    templateSteps: [
      "Şablonu indir ve Excel'de aç.",
      "Örnek rakamların üzerine kendi tutarlarını yaz; olmayan ayı boş bırak.",
      "Kaydet ve “Dosyayı Seç” ile geri yükle.",
    ],
    templateNote: "Kalem adlarını değiştirebilir, yenisini ekleyebilirsin. Ay adlarını olduğu gibi bırak — dosyayı onlardan tanıyoruz.",
    templateDownload: "Şablonu İndir",
    ...
    openingKept: "Kurulumda yazdığın ay başı bakiyesi",
    // Never reword: imported rows are recognised by this exact text
    // (`isWorkbookRemainderRow`), and every row already written carries it.
    ...
    recordsTitle: "Abonelikler ve yatırımlar",
    recordsHint: "Bu dosyadaki Abonelikler ve Yatırımlar sayfaları da gelir. Aynı ad ve döngüdeki abonelik ya da aynı gün, ürün ve adetteki işlem güncellenir, yenisi eklenir; hiçbir şey silinmez.",
    recordsCounts: (label: string, counts: { added: number; updated: number; unchanged: number }) =>
      `${label}: ${counts.added} yeni, ${counts.updated} güncellenecek, ${counts.unchanged} aynı`,
    recordsSubscriptions: "Abonelik",
    recordsInvestments: "Yatırım işlemi",
    recordsWalletMissing: "Yatırım işlemleri için önce Yatırımlar ekranında cüzdanını kur. Bu sayfa şimdilik atlanır.",
    recordsProblem: (sheet: string, row: number, column: string | null) =>
      column ? `${sheet}, ${row}. satır: ${column} okunamadı` : `${sheet}, ${row}. satır kaydedilemedi`,
    recordsMoreProblems: (count: number) => `ve ${count} satır daha`,
    recordsImport: "Abonelik ve Yatırımları Aktar",
    recordsDone: (subscriptions: number, investments: number) =>
      `${subscriptions} abonelik ve ${investments} yatırım işlemi eklendi ya da güncellendi.`,
