...
export const tr = {
  ...
  attachments: {
    ...
    unsupported: "Belge eklemek yalnızca telefon uygulamasında çalışır.",
    ...
    rowA11y: (name: string, kind: string, state: string) => `${kind}: ${name}. ${state}`,
    backupNote: "Yedek dosyası belgelerin kaydını taşır, içeriklerini taşımaz.",
  },
  statement: {
    title: "Ekstre Aktar",
    settingsDesc: "Kredi kartı ekstreni PDF'ten oku; her satırı onaylayarak aktar.",
    intro: "Kredi kartı ekstreni PDF olarak seç. Dosya cihazından çıkmaz; hiçbir yere yüklenmez.",
    pick: "PDF Seç",
    picking: "Okunuyor…",
    localOnly: "Ekstre yalnızca bu cihazda okunur. Helix PDF'i saklamaz, göndermez.",
    reviewTitle: "Okunanları Gözden Geçir",
    reviewHint: "Hiçbir satır sen onaylamadan kaydedilmez. Tutarı, tarihi ve kalemi düzeltebilirsin.",
  ...
  matching: {
    action: "Mevcut İşlemle Eşleştir",
    title: "Bu ödeme zaten kayıtlı mı?",
    hint: "Aynı parayı iki kez yazmamak için, bu beklenen ödemeyi kayıtlı bir işlemle eşleştirebilirsin.",
    none: "Yakın tarihte, uygun tutarda bir işlem bulunamadı.",
    exactAmount: "Tutar birebir aynı",
    closeAmount: "Tutar farklı",
    sameDay: "Aynı gün",
    dayGap: (days: number) => `${days} gün fark`,
    confirm: "Bunu Eşleştir",
    matched: "Beklenen ödeme kayıtlı işlemle eşleştirildi.",
    alreadyMatched: "Bu işlem başka bir beklenen ödemeyle eşleştirilmiş.",
    candidateA11y: (amount: string, date: string, reason: string) => `${amount}, ${date}. ${reason}.`,
  },
  duplicates: {
    title: "Olası tekrar kayıtlar",
    hint: "Aynı paranın iki kez yazılmış olabileceği satırlar. Helix hiçbirini kendiliğinden silmez.",
    certain: "Aynı kaynak satırı iki kez aktarılmış",
    suspected: "Tutar ve kalem aynı, tarihler yakın",
    none: "Tekrar eden kayıt görünmüyor.",
    open: "İşlemi Aç",
    sameDay: "aynı gün",
    dayGap: (days: number) => `${days} gün arayla`,
  },
  matrixColor: {
    title: (scope: string) => `${scope} rengi`,
    hint: (target: string) => `${target} için bir anlam seç. Renkler temanla birlikte açık ve koyu modda okunur kalır.`,
  ...
  investments: {
    ...
    setupWithExisting: "Aç ve Mevcut Yatırıma Geç",
    setupWithExistingHint: "Hesabı açar; ardından zaten sahip olduğun ürünü portföye kaydetme adımına geçirir. Serbest bakiyeden düşmez.",
    ...
    allocation: "Hedef dağılım",
    allocationHint: "Ürünlerin olması gereken payı ile şu anki payını karşılaştırır. Yatırım tavsiyesi değildir; hedefleri sen belirlersin.",
    allocationBasis: "Paylar, ürünlerin defter maliyetinden hesaplanır. Helix ürün bazında güncel piyasa değeri tutmaz; canlı fiyatlar yalnızca ana ekrandaki bilgi kartında gösterilir.",
    allocationEmpty: "Henüz hedef koymadın. Bir ürünü açıp hedef payını yazabilirsin.",
    allocationIncomplete: "Hedeflerin toplamı %100 değil; sapma yalnızca hedef koyduğun ürünler için hesaplandı.",
    allocationTotalDrift: (share: string) => `Toplam sapma ${share}`,
    allocationOnTarget: "Portföyün hedeflerinle uyumlu.",
    allocationTarget: "Hedef",
    allocationActual: "Şu an",
    allocationNoTarget: "Hedef yok",
    allocationOver: (share: string) => `${share} fazla`,
    allocationUnder: (share: string) => `${share} eksik`,
    allocationRowA11y: (name: string, actual: string, target: string, drift: string) =>
      `${name}. Şu an ${actual}, hedef ${target}. ${drift}.`,
    allocationTargetLabel: "Hedef pay (%)",
    allocationTargetHint: "Boş bırakırsan bu ürün için hedef tutulmaz.",
    allocationTargetInvalid: "0 ile 100 arasında tam bir yüzde yaz.",
